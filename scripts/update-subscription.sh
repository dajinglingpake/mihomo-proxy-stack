...
GEOSITE_FILE="${BASE_DIR}/config/GeoSite.dat"
...
curl -L --fail --max-time 60 --silent --show-error -A "clash.meta" "${SUBSCRIPTION_URL}" -o "${tmp_file}"
...
curl -L --fail --max-time 60 --silent --show-error "${GEOSITE_URL}" -o "${GEOSITE_FILE}"
